...
静态出口目标只有在对象已经存在、且不需要为地图主动 `load_object` 时才解析；未加载目标显示为“尚未探索”。函数、异常 mapping、随机/临时/动态出口显示为“未知出口/特殊通路”，`dynamic: true` 或 `resolved: false`。地图协议不调用 `valid_leave` 来探测通路，不触发房间 `reset`、NPC clone 或其他副作用。
...
地图视图放在 `features/map`，采用 React + CSS/SVG 小型局部布局，不引入大型地图引擎。中心节点表示当前真实房间，邻接节点按服务器方向 label 放置；`up/down` 和其他特殊出口使用独立出口列表，不假造二维坐标。未解析/动态出口可以看见状态，但不能伪装成已知稳定路线。
...
- `npm test -- --run`：16 个测试文件、62 个测试全部通过；`npm run build` 通过并更新 `www/app` 构建产物。
- 重启后的 `bin/driver.exe config.ini -d` 实例成功加载新 `feature/user_gmcp`，未再出现本轮代码产生的编译警告。真实 WebSocket 登录后收到 `Room.Info`、`Room.Map`、Terminal 和 `Room.Entities`；从客店茶房走过客店、北大街、中央广场、南大街，再进入树洞并原路出去，共收到 8 次地图快照、6 个真实房间节点。
- 在真实 WebSocket 上先提交伪造的 `x-invalid-999`，服务器拒绝；再使用服务器签发的 opaque 出口 token 从北大街进入中央广场，换房后重放旧 token，服务器再次拒绝。地图点击没有提交方向或 LPC 路径。
- 真实 UI 在 390×844、844×390、1440×900 下检查无横向溢出；地图页显示当前节点、已解析邻接节点和独立的 `进入`/`出去` 通路，未解析出口保持不可点击。Telnet 两个端口的 `look`、`north`、`south` 均到达原版命令处理链，未见本轮新增 GMCP 编译错误。
- 一次性角色及其 `data/login/s`、`data/user/s` 存档和本轮调试/编译日志已删除；未修改 `mudcore`，未保留测试凭据或玩家存档。
