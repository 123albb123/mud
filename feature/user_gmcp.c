...
private nosave mapping gmcp_map_exit_records = ([]);
private nosave string gmcp_map_exit_session;
...
private nosave mapping gmcp_chat_target_ids = ([]);
private nosave string gmcp_chat_target_session;
private nosave int gmcp_chat_target_sequence;
private nosave int gmcp_chat_targets_revision;
private nosave string gmcp_chat_targets_fingerprint;
private nosave int gmcp_chat_targets_refresh_pending;
private nosave int gmcp_chat_targets_polling;
...
void gmcp_chat_targets_changed();
...
void gmcp_chat_private_message(string kind, object sender, object recipient,
                               string text);
void gmcp_chat_private_delivered(string kind, string sender_name,
...
private int gmcp_room_is_area(object room)
{
    ...
    if (!objectp(room) || !function_exists("is_area", room))
        return 0;
    if (catch(result = call_other(room, "is_area")))
        return 0;
    return intp(result) && result != 0;
}

private string query_gmcp_room_id_for_key(string key)
{
    if (!stringp(key) || key == "")
        return "";
...
private string query_gmcp_room_id_at(object room, int x, int y,
                                     int use_coordinate)
{
    string key;
    ...
    key = file_name(room);
    if (use_coordinate && gmcp_room_is_area(room))
        key += sprintf("@%d,%d", x, y);
    return query_gmcp_room_id_for_key(key);
}
...
private string query_gmcp_room_id(object room)
{
    ...
    if (gmcp_room_is_area(room) && room == environment(this_object()))
    {
        info = 0;
        catch(info = this_object()->query("area_info"));
        if (mapp(info) && intp(info["x_axis"]) && intp(info["y_axis"]))
            return query_gmcp_room_id_at(room, info["x_axis"],
                                         info["y_axis"], 1);
    }
...
private string gmcp_strip_ansi(string value)
{
    string result;
    int length;
    int i;
    ...
    for (i = 0; i < length; i++)
    {
        ...
        {
            i++;
            while (i < length && value[i] != 7)
            {
                if (value[i] == 27 && i + 1 < length &&
                    value[i + 1] == '\\')
                {
                    i++;
                    break;
...
private string gmcp_task_text(mixed value, int limit, int multiline)
{
    ...
    text = gmcp_strip_ansi(remove_ansi(value));
    text = replace_string(text, "\r", "");
    if (!multiline)
...
private mixed gmcp_map_query(object room, string key)
{
    mixed value;

    if (!objectp(room) || !stringp(key))
        return 0;
    value = 0;
    if (catch(value = room->query(key)))
        return 0;
    return value;
}

private string gmcp_map_text(mixed value, int limit)
{
    string text;
    string result;
    int i;

    text = gmcp_task_text(value, limit, 0);
    result = "";
...
private mapping gmcp_map_current_area_info(object room)
{
    mapping info;
...
private string gmcp_map_room_name(object room, int x, int y, int use_coordinate)
{
    mixed value;

    value = 0;
    if (use_coordinate && function_exists("query_data", room))
        catch(value = call_other(room, "query_data", x, y, "short"));
    if (!stringp(value) || value == "")
        value = gmcp_map_query(room, "short");
    if (!stringp(value) || value == "")
        value = gmcp_map_query(room, "name");
    return gmcp_map_text(value, GMCP_MAP_TEXT_LIMIT);
}

private string gmcp_map_area_name(object room)
{
    mixed value;

    value = gmcp_map_query(room, "outdoors");
    if (!stringp(value) || value == "")
        value = gmcp_map_query(room, "area");
    if ((!stringp(value) || value == "") && gmcp_room_is_area(room))
        value = gmcp_map_query(room, "name");
    return gmcp_map_text(value, GMCP_MAP_TEXT_LIMIT);
}

private mapping gmcp_map_room_record(object room, int x, int y,
                                     int use_coordinate)
{
    mapping record;
    string room_id;
    ...
    room_id = use_coordinate
        ? query_gmcp_room_id_at(room, x, y, 1)
        : query_gmcp_room_id(room);
    if (room_id == "")
        return 0;

    record = ([
        "room_id": room_id,
        "name"   : gmcp_map_room_name(room, x, y, use_coordinate),
    ]);
    area = gmcp_map_area_name(room);
    if (area != "")
        record["area"] = area;
    return record;
}

private mapping gmcp_map_direction_labels()
{
    return ([
        "north"     : "北",
        "south"     : "南",
...
private string gmcp_map_exit_label(string command)
{
    mapping labels;
    string label;

    labels = gmcp_map_direction_labels();
    label = labels[command];
    if (stringp(label) && label != "")
        return label;
    label = gmcp_map_text(command, GMCP_MAP_TEXT_LIMIT);
    return label == "" ? "特殊通路" : label;
}

private string gmcp_map_exit_kind(string command)
{
    if (command == "up" || command == "down" ||
        command == "northup" || command == "southup" ||
        command == "eastup" || command == "westup" ||
        command == "northdown" || command == "southdown" ||
        command == "eastdown" || command == "westdown")
        return "vertical";
    if (command == "enter" || command == "in" || command == "out")
        return "portal";
...
private int gmcp_map_command_allowed(mixed value)
{
    string command;
    int i;
    ...
    if (!stringp(value))
        return 0;
    command = value;
    if (command == "" || strlen(command) > GMCP_MAP_COMMAND_LIMIT ||
        strsrch(command, "/") != -1 || strsrch(command, "\\") != -1)
        return 0;
...
private mapping gmcp_map_area_neighbor(string command, int x, int y)
{
    mapping result;
    int target_x;
    int target_y;
...
private int gmcp_map_exit_conditional(object room, string command)
{
    mapping doors;
    mixed guarded;
    ...
    guarded = 0;
    catch(guarded = room->query_temp("guarded/" + command));
    return arrayp(guarded) && sizeof(guarded) > 0;
}

private mapping gmcp_map_base_exit(object room, string command)
{
    mapping record;

    record = ([
        "command"        : gmcp_map_text(command, GMCP_MAP_TEXT_LIMIT),
        "label"           : gmcp_map_exit_label(command),
        "kind"            : gmcp_map_exit_kind(command),
        "resolved"        : 0,
        "dynamic"         : 0,
    ...
    if (record["command"] == "")
        record["command"] = "特殊通路";
    if (gmcp_map_exit_conditional(room, command))
        record["conditional"] = 1;
    return record;
}

private mapping gmcp_map_resolve_destination(mixed destination)
{
    mapping result;
    object target;
    string path;
    ...
    if (mapp(destination))
    {
        ...
        if (!stringp(path) || path == "" || strlen(path) > 240 ||
            !intp(destination["x_axis"]) || !intp(destination["y_axis"]) ||
            strsrch(path, "\n") != -1 || strsrch(path, "\r") != -1)
        {
            result["dynamic"] = 1;
            return result;
        ...
        result["resolved"] = 1;
        result["object"] = target;
        result["has_coordinate"] = 1;
        result["x_axis"] = destination["x_axis"];
        result["y_axis"] = destination["y_axis"];
        return result;
    }
    result["dynamic"] = 1;
    return result;
...
private string gmcp_map_destination_name(object room, int x, int y,
                                          int use_coordinate)
{
    return gmcp_map_room_name(room, x, y, use_coordinate);
}

private void gmcp_map_fill_destination(mapping record, mapping destination)
{
    object target;
    string room_id;
    string name;
    ...
    if (!mapp(record) || !mapp(destination))
        return;
    ...
    if (!destination["resolved"] || !objectp(target = destination["object"]))
        return;

    has_coordinate = intp(destination["has_coordinate"]) &&
                     destination["has_coordinate"] != 0;
    x = destination["x_axis"];
    y = destination["y_axis"];
    if (gmcp_room_is_area(target))
    {
        if (!has_coordinate)
        {
            record["dynamic"] = 1;
        ...
        room_id = query_gmcp_room_id_at(target, x, y, 1);
        name = gmcp_map_destination_name(target, x, y, 1);
    }
    else
    {
        room_id = query_gmcp_room_id(target);
        name = gmcp_map_destination_name(target, 0, 0, 0);
    }
    if (room_id == "")
        return;

    record["resolved"] = 1;
    record["destination_room_id"] = room_id;
    if (name != "")
        record["destination_name"] = name;
}

private mapping gmcp_map_room_exit_record(object room, string command,
                                          mixed destination)
{
    mapping record;
    mapping destination_info;

    record = gmcp_map_base_exit(room, command);
    destination_info = gmcp_map_resolve_destination(destination);
    gmcp_map_fill_destination(record, destination_info);
    return record;
}

private mapping gmcp_map_area_exit_record(object room, string command,
                                          int x, int y)
{
    mapping record;
    mapping neighbor;
    ...
    record = gmcp_map_base_exit(room, command);
    neighbor = gmcp_map_area_neighbor(command, x, y);
    if (!mapp(neighbor))
        return record;

    cell = 0;
    catch(cell = room->query_info(neighbor["x_axis"], neighbor["y_axis"]));
    if (!mapp(cell))
        return record;

    if (!undefinedp(cell["room_exit"]))
    {
        raw_exit = cell["room_exit"];
        if (!stringp(raw_exit))
        {
            record["dynamic"] = 1;
            return record;
        ...
        path = "";
        catch(path = room->file_path(raw_exit));
        if (!stringp(path) || path == "")
            path = raw_exit;
        destination_info = gmcp_map_resolve_destination(path);
        gmcp_map_fill_destination(record, destination_info);
        return record;
    }

    if (!undefinedp(cell["area_exit"]))
    {
        if (!mapp(cell["area_exit"]) ||
            !stringp(cell["area_exit"]["filename"]) ||
            !intp(cell["area_exit"]["x_axis"]) ||
            !intp(cell["area_exit"]["y_axis"]))
        {
            record["dynamic"] = 1;
            return record;
        }
        path = "";
        catch(path = room->file_path(cell["area_exit"]["filename"]));
        if (!stringp(path) || path == "")
            path = cell["area_exit"]["filename"];
        destination = ([
            "filename": path,
            "x_axis"  : cell["area_exit"]["x_axis"],
            "y_axis"  : cell["area_exit"]["y_axis"],
        ]);
        destination_info = gmcp_map_resolve_destination(destination);
        gmcp_map_fill_destination(record, destination_info);
        return record;
    ...
    record["resolved"] = 1;
    record["destination_room_id"] = query_gmcp_room_id_at(
        room, neighbor["x_axis"], neighbor["y_axis"], 1);
    record["destination_name"] = gmcp_map_destination_name(
        room, neighbor["x_axis"], neighbor["y_axis"], 1);
    return record;
}

private string *gmcp_map_room_commands(object room)
{
    mapping area_info;
    mapping exits;
    mixed *exit_keys;
    ...
    else
    {
        ...
        for (i = 0; i < sizeof(exit_keys); i++)
        {
            if (stringp(exit_keys[i]))
                commands += ({exit_keys[i]});
        }
    }

    commands = filter_array(commands, (: gmcp_map_command_allowed($1) :));
    commands = sort_array(commands, (: strcmp($1, $2) :));
    return commands;
}

private mapping gmcp_map_snapshot()
{
    object room;
    mapping area_info;
    mapping exits;
    ...
    area_info = gmcp_map_current_area_info(room);
    current = gmcp_map_room_record(room,
    ...
    records = ({});
    commands = gmcp_map_room_commands(room);
    if (mapp(area_info))
    {
        for (i = 0; i < sizeof(commands) &&
             sizeof(records) < GMCP_MAP_EXIT_LIMIT; i++)
        {
            record = gmcp_map_area_exit_record(room, commands[i],
                                               area_info["x_axis"],
                                               area_info["y_axis"]);
            if (mapp(record))
                records += ({record});
    ...
    else
    {
        ...
        {
            command = commands[i];
            record = gmcp_map_room_exit_record(room, command, exits[command]);
            if (mapp(record))
                records += ({record});
        }
    }
...
private mapping gmcp_map_public_snapshot(mapping snapshot)
{
    mapping result;
    mapping record;
    mixed *records;
    mixed *public_records;
    int i;
    ...
    if (pointerp(records))
    {
        ...
        {
            if (!mapp(record = records[i]))
                continue;
            record = copy(record);
            map_delete(record, "_server_command");
            public_records += ({record});
        }
    }
    result["exits"] = public_records;
    return result;
}

private string query_gmcp_map_exit_id()
{
    if (!stringp(gmcp_map_exit_session))
        gmcp_map_exit_session = sprintf("%08x", random(0x7fffffff));
    gmcp_map_exit_sequence++;
    return sprintf("x-%s-%04d", gmcp_map_exit_session,
                   gmcp_map_exit_sequence);
}
...
{
    mapping snapshot;
    mapping public_snapshot;
    mapping record;
    mapping public_record;
    mixed *records;
    mixed *public_records;
    string fingerprint;
    string exit_id;
    ...
    snapshot = gmcp_map_snapshot();
    public_snapshot = gmcp_map_public_snapshot(snapshot);
    if (!mapp(snapshot) || !mapp(public_snapshot))
        return;

    fingerprint = gmcp_state_fingerprint(public_snapshot);
    if (!force && gmcp_map_revision > 0 &&
        fingerprint == gmcp_map_fingerprint)
        return;
    if (fingerprint != gmcp_map_fingerprint || gmcp_map_revision == 0)
        gmcp_map_revision++;
    ...
    for (i = 0; i < sizeof(records) && i < sizeof(public_records); i++)
    {
        record = records[i];
        public_record = public_records[i];
        if (!mapp(record) || !mapp(public_record))
            continue;
        exit_id = query_gmcp_map_exit_id();
        command = record["_server_command"];
        if (!stringp(command))
            continue;
        public_record["exit_id"] = exit_id;
        gmcp_map_exit_records[exit_id] = ([
            "room"    : room,
...
private int gmcp_safe_map_exit_id(string value)
{
    ...
    if (!stringp(value) || strlen(value) < 5 || strlen(value) > 96 ||
        value[0] != 'x' || value[1] != '-')
        return 0;
...
private void gmcp_map_action_failed(string message)
{
    write(message + "\n");
...
private void gmcp_handle_web_room_move(string payload)
{
    ...
    {
        gmcp_map_action_failed("地图移动请求无效。");
        return;
    }
    data = decoded;
    if (sizeof(keys(data)) != 1 ||
        !stringp(exit_id = data["exit_id"]) ||
        !gmcp_safe_map_exit_id(exit_id))
    {
        gmcp_map_action_failed("地图出口请求无效。");
        return;
    }

    record = gmcp_map_exit_records[exit_id];
    room = environment(this_object());
    if (!mapp(record) || !objectp(room) || record["room"] != room ||
        record["revision"] != gmcp_map_revision ||
        record["room_id"] != query_gmcp_room_id(room))
    {
        gmcp_map_action_failed("地图出口已经失效，请等待当前房间同步。");
        return;
    }

    move_command = record["command"];
    if (!gmcp_map_command_allowed(move_command))
    {
    ...
    if (!result)
    {
        gmcp_write_native_action_failure("地图移动未能完成，请查看原有游戏文字。");
        gmcp_refresh_room_map(0);
        return;
    }
    gmcp_refresh_room_map(0);
...
private int gmcp_quest_inventory_count(string target_file)
{
    object *inventory;
    object item;
    string item_file;
    ...
    if (!stringp(target_file) || target_file == "")
        return 0;
    ...
    if (mapp(requirements))
    {
        ...
        for (i = 0; i < sizeof(keys_to_check); i++)
        {
            ...
            current = gmcp_quest_progress(progress, "killed",
                                          keys_to_check[i]);
            if (status == "completed" && gmcp_task_number(required) &&
                (!gmcp_task_number(current) || current < required))
                current = required;
    ...
    if (mapp(requirements))
    {
        ...
        for (i = 0; i < sizeof(keys_to_check); i++)
        {
            ...
            current = gmcp_quest_progress(progress, "item",
                                          keys_to_check[i]);
            inventory_current = gmcp_quest_inventory_count(keys_to_check[i]);
            if (!gmcp_task_number(current) || inventory_current > current)
...
private int gmcp_chat_target_eligible(object me, object target)
{
    int is_player;
    int is_chatter;
    int is_net_dead;
    ...
    if (!objectp(me) || !objectp(target) || target == me ||
        !userp(target) || !interactive(target) || !living(target))
        return 0;
    if (catch(is_player = playerp(target)) || !is_player)
        return 0;
    if (function_exists("is_chatter", target) &&
        !catch(is_chatter = (int)target->is_chatter()) && is_chatter)
        return 0;
    if (function_exists("is_net_dead", target) &&
        !catch(is_net_dead = (int)target->is_net_dead()) && is_net_dead)
        return 0;
    if (catch(visible = me->visible(target)) || !visible)
        return 0;
    return 1;
}

private string query_gmcp_chat_target_id(object target)
{
    string key;
    ...
    if (!mapp(gmcp_chat_target_ids))
        gmcp_chat_target_ids = ([]);
    if (!stringp(gmcp_chat_target_session))
        gmcp_chat_target_session = sprintf("%08x", random(0x7fffffff));

    key = file_name(target);
    if (!stringp(gmcp_chat_target_ids[key]))
    {
        gmcp_chat_target_sequence++;
        gmcp_chat_target_ids[key] = sprintf("p-%s-%04d",
                                             gmcp_chat_target_session,
                                             gmcp_chat_target_sequence);
    }
    return gmcp_chat_target_ids[key];
}

private string gmcp_chat_target_name(object target)
{
    mixed value;
    string name;

    name = "";
    if (objectp(target) && !catch(value = target->name(1)) &&
        stringp(value))
        name = gmcp_chat_text(value);
    if (name == "" && objectp(target) &&
        !catch(value = target->short()) && stringp(value))
        name = gmcp_chat_text(value);
    if (strlen(name) > 256)
        name = name[0..255];
    return name;
...
private mapping gmcp_chat_target_record(object target)
{
    mapping record;
    string public_id;
    mixed value;

    if (!gmcp_chat_target_eligible(this_object(), target))
        return 0;
    if (gmcp_chat_target_name(target) == "")
        return 0;

    record = ([
        "player_id": query_gmcp_chat_target_id(target),
        "name"     : gmcp_chat_target_name(target),
    ]);
    public_id = "";
    if (!catch(value = target->query("id")) && stringp(value))
        public_id = gmcp_chat_text(value);
    if (strlen(public_id) > 128)
        public_id = public_id[0..127];
    if (public_id != "" && strsrch(public_id, "/") == -1 &&
        strsrch(public_id, "\\") == -1)
        record["id"] = public_id;
    return record;
}

private mapping gmcp_chat_targets_snapshot()
{
    object *online;
    mixed *players;
    mapping active;
    ...
    if (mapp(gmcp_chat_target_ids))
    {
        keys_to_check = keys(gmcp_chat_target_ids);
        for (i = 0; i < sizeof(keys_to_check); i++)
        {
            key = keys_to_check[i];
            if (!active[key])
                map_delete(gmcp_chat_target_ids, key);
        }
    }
    ...
    return ([
        ...
        "snapshot": 1,
        "revision": gmcp_chat_targets_revision,
        "sequence": gmcp_chat_targets_revision,
        "players" : players,
    ]);
}

varargs void gmcp_refresh_chat_targets(int force)
{
    mapping targets;
    string fingerprint;

    if (!has_gmcp() || (!force && !gmcp_supports("Chat.Targets")))
        return;

    targets = gmcp_chat_targets_snapshot();
    fingerprint = gmcp_state_fingerprint(targets);
    if (!force && fingerprint == gmcp_chat_targets_fingerprint)
        return;
    if (fingerprint != gmcp_chat_targets_fingerprint)
        gmcp_chat_targets_revision++;
    targets["revision"] = gmcp_chat_targets_revision;
    targets["sequence"] = gmcp_chat_targets_revision;
    sendGMCP(targets, "Chat", "Targets");
    gmcp_chat_targets_fingerprint = fingerprint;
}

void gmcp_flush_chat_targets()
{
    gmcp_chat_targets_refresh_pending = 0;
    gmcp_refresh_chat_targets();
}

void gmcp_chat_targets_changed()
{
    if (!interactive(this_object()) || !has_gmcp() ||
        !gmcp_supports("Chat.Targets") ||
        gmcp_chat_targets_refresh_pending)
        return;
    gmcp_chat_targets_refresh_pending = 1;
    call_out("gmcp_flush_chat_targets", 0);
}

void gmcp_poll_chat_targets()
{
    gmcp_chat_targets_polling = 0;
    if (!interactive(this_object()) || !has_gmcp() ||
        !gmcp_supports("Chat.Targets"))
        return;
    gmcp_refresh_chat_targets();
    gmcp_chat_targets_polling = 1;
    call_out("gmcp_poll_chat_targets", GMCP_CHAT_TARGET_POLL_INTERVAL);
}

private void gmcp_start_chat_target_poll()
{
    if (!interactive(this_object()) || !has_gmcp() ||
        !gmcp_supports("Chat.Targets") || gmcp_chat_targets_polling)
        return;
    gmcp_chat_targets_polling = 1;
    call_out("gmcp_poll_chat_targets", GMCP_CHAT_TARGET_POLL_INTERVAL);
}

private int gmcp_safe_chat_player_id(string value)
{
    int i;
...
private object gmcp_find_chat_target(string player_id)
{
    object *online;
    string key;
    int i;

    if (!gmcp_safe_chat_player_id(player_id) ||
        !mapp(gmcp_chat_target_ids))
        return 0;

    online = all_interactive();
    for (i = 0; i < sizeof(online); i++)
    {
        if (!gmcp_chat_target_eligible(this_object(), online[i]))
            continue;
        key = file_name(online[i]);
        if (gmcp_chat_target_ids[key] == player_id)
            return online[i];
    }
    return 0;
}
                                 ...
                                 string sender_id, string text)
{
    if (!interactive(this_object()) || !has_gmcp() ||
        ...
        (kind != "tell" && kind != "reply"))
        return;
    gmcp_chat_send_event(kind, text, "", 0, sender_name, sender_id,
                         this_object(), 0);
}
...
private void gmcp_reset_session_state()
{
    ...
    remove_call_out("gmcp_flush_chat_targets");
    remove_call_out("gmcp_poll_chat_targets");

    gmcp_room_ids = ([]);
    gmcp_room_session = 0;
    gmcp_room_sequence = 0;
    gmcp_map_exit_records = ([]);
    gmcp_map_exit_session = 0;
    ...
    gmcp_chat_capabilities_refresh_pending = 0;
    gmcp_chat_target_ids = ([]);
    gmcp_chat_target_session = 0;
    gmcp_chat_target_sequence = 0;
    gmcp_chat_targets_revision = 0;
    gmcp_chat_targets_fingerprint = 0;
    gmcp_chat_targets_refresh_pending = 0;
    gmcp_chat_targets_polling = 0;
...
private void gmcp_enable()
{
    ...
    sendGMCP(([
        ...
        "supports": ([
            ...
            "Chat.Targets"   : GMCP_CHAT_TARGETS_VERSION,
        ]),
    ]), "Server", "Hello");
}
...
private void gmcp_handle_web_chat_send(string payload)
{
    ...
    {
        has_entity_target = !undefinedp(data["target_entity_id"]);
        has_player_target = !undefinedp(data["target_player_id"]);
        if (has_entity_target == has_player_target)
        {
        ...
        if (has_player_target)
        {
            ...
            if (!stringp(target_player_id) ||
                !gmcp_safe_chat_player_id(target_player_id))
            {
                gmcp_write_native_action_failure("聊天目标无效。");
                return;
            }
            target = gmcp_find_chat_target(target_player_id);
        }
        else
        {
            target_entity_id = data["target_entity_id"];
...
void gmcp(string req)
{
    ...
    else if (package == "Room.Map.Get")
    {
        gmcp_refresh_room_map(1);
    }
    ...
    else if (package == "Chat.Targets.Get")
    {
        gmcp_refresh_chat_targets(1);
        gmcp_start_chat_target_poll();
    }
    ...
    else if (package == "Web.Room.Move")
    {
        gmcp_handle_web_room_move(payload);
    }
}
